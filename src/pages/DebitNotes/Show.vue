      ...
      <div class="mx-auto max-w-3xl sm:px-6 lg:px-8">
        ...
        <div v-else class="bg-white rounded-lg shadow p-6 space-y-4 text-sm">
          <div class="grid grid-cols-2 gap-4">
            <div><span class="text-gray-500">Purchase Bill</span><div class="mt-0.5"><RouterLink :to="'/purchases/' + note.purchase_id" class="text-indigo-600 hover:underline">{{ note.bill_number }}</RouterLink></div></div>
            <div><span class="text-gray-500">Supplier</span><div class="font-medium mt-0.5">{{ note.supplier_name }}</div></div>
            ...
            <div><span class="text-gray-500">Amount</span><div class="font-bold text-orange-600 mt-0.5">₹{{ fmt(note.amount) }}</div></div>
...
async function loadData() {
    loading.value = true
    try { const { data } = await api.get('/debit-notes/' + route.params.id); note.value = data.debitNote } finally { loading.value = false }
}
