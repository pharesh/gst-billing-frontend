  ...
  <AuthenticatedLayout>
    <template #header>
      <div class="flex items-center justify-between">
        <h2 class="text-xl font-semibold text-gray-800">New Debit Note</h2>
        ...
        <form v-else @submit.prevent="save" class="space-y-4">
          ...
          <div class="bg-white rounded-lg shadow p-6">
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="text-sm font-medium text-gray-700 block mb-1">Purchase Bill *</label>
                <select v-model="form.purchase_id" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                  <option value="">Select purchase</option>
                  <option v-for="p in purchases" :key="p.id" :value="p.id">{{ p.bill_number }} — {{ p.supplier_name }}</option>
                </select>
                <p v-if="errors.purchase_id" class="text-red-500 text-xs mt-1">{{ errors.purchase_id[0] }}</p>
              </div>
              <div>
          ...
          <div class="flex gap-3 justify-end">
            <RouterLink to="/debit-notes" class="px-4 py-2 border border-gray-300 text-sm rounded-lg hover:bg-gray-50">Cancel</RouterLink>
            <button type="submit" :disabled="processing" class="px-6 py-2 bg-indigo-600 text-white text-sm rounded-lg hover:bg-indigo-700 disabled:opacity-50">{{ processing ? 'Saving…' : 'Create Debit Note' }}</button>
          </div>
        </form>
      </div>
    </div>
...
const loading = ref(true); const purchases = ref([])
const now = new Date().toISOString().slice(0, 10)
const form = reactive({ purchase_id: '', note_date: now, amount: '', gst_amount: 0, reason: '' })

async function loadData() {
    const { data } = await api.get('/debit-notes/create')
    purchases.value = data.purchases ?? []; loading.value = false
}
async function save() {
    await submit('post', '/debit-notes', form, { onSuccess: d => router.push('/debit-notes/' + d.debitNote?.id) })
}
onMounted(loadData)
</script>
